// filepath: /Users/adityanbhatt/Documents/Data-inisights-copilot/frontend/src/pages/FileDataPage.tsx
/**
 * FileDataPage - Manage uploaded file tables for SQL/RAG queries
 * 
 * Features:
 * - List all uploaded file tables with metadata
 * - View table schema and preview data
 * - Configure RAG embedding for text columns
 * - Delete tables
...
export default function FileDataPage() {
  ...
  const [activeTab, setActiveTab] = useState<'schema' | 'preview' | 'rag'>('schema');
  ...
  const loadTableDetails = async (tableName: string) => {
    ...
    try {
      // Load schema, preview, classification, and RAG status in parallel
  ...
  const handleRAGStatusChange = (status: RAGStatus | null) => {
    ...
    // Refresh capabilities when RAG status changes
    loadCapabilities();
  };

  const handleRAGError = (errorMsg: string) => {
    setError(errorMsg);
  };
    ...
    <div className="h-screen flex flex-col bg-gray-50 overflow-hidden">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex-shrink-0">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">File Data Explorer</h1>
            <p className="text-sm text-gray-500 mt-1">
              Manage uploaded files and configure SQL/RAG queries
            </p>
          </div>
          <button
            onClick={loadTables}
            ...
            <div className="col-span-8">
              {!selectedTable ? (
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                  ...
                  <div className="p-6">
                    {/* Schema Tab */}
                    {activeTab === 'schema' && tableDetails?.schema && (
                      <div>
                    ...
                    {/* RAG Config Tab */}
                    {activeTab === 'rag' && selectedTable && (
                      <RAGConfigPanel
                        tableName={selectedTable}
                        onStatusChange={handleRAGStatusChange}
                        onError={handleRAGError}
                      />
                    )}
                  </div>
                </div>
