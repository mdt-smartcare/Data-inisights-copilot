...
- Use for: "Find records related to...", "Show me details about..."."""
    ...
    async def _rewrite_query_with_context(self, query: str, session_id: Optional[str]) -> str:
        ...
        import os
        try:
            template_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "agent_spec", "prompt_templates", "query_rewriter.md")
            with open(template_path, "r") as f:
                system_prompt = f.read()
        except Exception:
            system_prompt = "You are a query rewriter. Rewrite the query to be self-contained using history."
